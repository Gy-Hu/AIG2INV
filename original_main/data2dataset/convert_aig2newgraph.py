# HZ: this is different from aig2graph as it uses aag2adjmat.py
...
def convert_aag(aag_fname, inv_cnf):
  ...
  graph = aagmodel.to_dataframe(7, clauses.clauses, aag_fname) # 4 is the type of node (input, )
  total_time = time.time() - start_time
...
def test():
  all_graphs = [ \
    convert_aag(aag_fname="testcase1/cnt.aag", inv_cnf="testcase1/inv.cnf")[0], \
    convert_aag(aag_fname="testcase2/cnt2.aag", inv_cnf="testcase2/inv.cnf")[0], \
    ]
  # print (all_graphs[0].forward_layer_index)
  print (all_graphs[0].edge_index)
  with open("newgraph/test.pkl", 'wb') as f:
      pickle.dump(all_graphs, f)
      

def test_zero():
  all_graphs = [ \
    convert_aag(aag_fname="testcase3-zeros/cnt.aag", inv_cnf="testcase3-zeros/inv.cnf")[0], \
    ]
  # print (all_graphs[0].forward_layer_index)
  print (all_graphs[0].edge_index)
  with open("newgraph/test3zeros.pkl", 'wb') as f:
      pickle.dump(all_graphs, f)
      
def test_intel001():
  all_graphs = [ \
    convert_aag(aag_fname="testcase1/cnt.aag", inv_cnf="testcase1/inv.cnf")[0], \
    convert_aag(aag_fname="../hwmcc07-mod/intel/intel_001.aag", inv_cnf="../hwmcc07-7200-result/output/intel/intel_001/inv.cnf")[0], \
    convert_aag(aag_fname="testcase2/cnt2.aag", inv_cnf="testcase2/inv.cnf")[0], \
    ]
  ...
  with open("newgraph/test_intel001.pkl", 'wb') as f:
      pickle.dump(all_graphs, f)
...
if __name__ == '__main__':
    ...
    parser.add_argument('dataset', action='store', type=str, default='newgraph/hwmcc07dataset.pkl')
    ...
    for aagname in allfile:
      ...
      print (aag_fname, inv_cnf, end = ' Time: ', flush=True)
      g, dt = convert_aag(aag_fname, inv_cnf)
      print (dt)
      all_graphs.append(g)
